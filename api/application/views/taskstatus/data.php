...
<?php
 }else if(!IsAllowView(MODULTASKSTATUS)){
 ?>		
 	<div class='alert alert-warning'>
...
<?	
	...
	if(!IsAllowDelete(MODULTASKSTATUS)){
?>			
		<style>
			.cekboxaction{
				display:none;
...
<?php		
	...
	foreach ($c->result_array() as $d){
		...
		$data[$i] = array(
						'<input type="checkbox" class="cekbox" name="cekbox[]" value="'.$d[COL_TASKSTATUSID].'" />',
						$d[COL_TASKSTATUSID],
						anchor('taskstatus/edit/'.$d[COL_TASKSTATUSID],$d[COL_TASKSTATUSNAME]),
						$d[COL_ORDER]
		...
		<div class="row">
			<div class="col-md-12">
				<h1><?= $title ?></h1>
					...
					<p>
						<?=anchor('taskstatus/delete','<i class="clip-remove"></i> Hapus',array('class'=>'cekboxaction ui btn btn-danger','confirm'=>'Apa anda yakin?'))?>
...
$(document).ready(function(){
		...
		var pelangganTable = $('#databerita').dataTable({
	        ...
	        "aoColumns": [
	        	...
	        	{"sTitle": "ID Jenis Tugas"},
	            {"sTitle": "Nama jenis Tugas"},
	            {"sTitle": "Order"}
	        ],
