...
<?php
 }else if(!IsAllowView(MODULTASKTYPE)){
 ?>		
 	<div class='alert alert-warning'>
...
<?	
	...
	if(!IsAllowDelete(MODULTASKTYPE)){
?>			
		<style>
			.cekboxaction{
				display:none;
...
<?php		
	...
	foreach ($c->result_array() as $d){
		...
		$data[$i] = array(
						'<input type="checkbox" class="cekbox" name="cekbox[]" value="'.$d[COL_TASKTYPEID].'" />',
						$d[COL_TASKTYPEID],
						anchor('tasktype/edit/'.$d[COL_TASKTYPEID],$d[COL_TASKTYPENAME]),
						$d[COL_ORDER]
		...
		<div class="row">
			<div class="col-md-12">
				<h1><?= $title ?></h1>
					...
					<p>
						<?=anchor('tasktype/delete','<i class="clip-remove"></i> Hapus',array('class'=>'cekboxaction ui btn btn-danger','confirm'=>'Apa anda yakin?'))?>
