...
<?php
 }else if(!IsAllowView(MODULTASKRESOLUTION)){
 ?>		
 	<div class='alert alert-warning'>
...
<?	
	...
	if(!IsAllowDelete(MODULTASKRESOLUTION)){
?>			
		<style>
			.cekboxaction{
				display:none;
...
<?php		
	...
	foreach ($c->result_array() as $d){
		...
		$data[$i] = array(
						'<input type="checkbox" class="cekbox" name="cekbox[]" value="'.$d[COL_TASKRESOLUTIONID].'" />',
						$d[COL_TASKRESOLUTIONID],
						anchor('taskresolution/edit/'.$d[COL_TASKRESOLUTIONID],$d[COL_TASKRESOLUTIONNAME]),
						$d[COL_ORDER]
		...
		<div class="row">
			<div class="col-md-12">
				<h1><?= $title ?></h1>
					...
					<p>
						<?=anchor('taskresolution/delete','<i class="clip-remove"></i> Hapus',array('class'=>'cekboxaction ui btn btn-danger','confirm'=>'Apa anda yakin?'))?>
						<!-- <?= anchor('taskresolution/add','Tambah', array('class'=>'btn btn-default'))?> -->
					</p>
...
$(document).ready(function(){
		...
		var pelangganTable = $('#databerita').dataTable({
	        ...
	        "aoColumns": [
	        	...
	        	{"sTitle": "ID Resolusi Tugas"},
	            {"sTitle": "Nama Resolusi Tugas"},
	            {"sTitle": "Order"}
	        ],
