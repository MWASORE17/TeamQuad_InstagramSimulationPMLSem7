...
<?php		
	...
	foreach ($c->result_array() as $d){
		...
		$data[$i] = array(
						'<input type="checkbox" class="cekbox" name="cekbox[]" value="'.$d[COL_NEWSID].'" />',
						$d[COL_NEWSID],
						anchor('news/edit/'.$d[COL_NEWSID],$d[COL_NEWSTITLE]),
						$d[COL_STARTEDDATE],
						$d[COL_EXPIREDDATE],
						$d[COL_CREATEDBY]
					);
		...
		<div class="row">
			<div class="col-md-12">
				<h1><?= $title ?></h1>
					...
					<p>
						<?=anchor('news/delete','<i class="clip-remove"></i> Hapus',array('class'=>'cekboxaction ui btn btn-danger','confirm'=>'Apa anda yakin?'))?>
						<!-- <?= anchor('user/add','Tambah', array('class'=>'btn btn-default'))?> -->
					</p>
